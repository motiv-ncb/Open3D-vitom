...
Eigen::Matrix4d TransformationEstimationPointToPlane::ComputeTranslation(
    const geometry::PointCloud &source,
    const geometry::PointCloud &target,
    const CorrespondenceSet &corres) const {
    if (corres.empty() || !target.HasNormals())
    ...
    double & r, double & w) {
        ...
        // Set rotation component of Jacobian to zero
    ...
    // Extract translation and form a matrix with identity rotation and the obtained translation
    Eigen::Matrix4d translation_only_extrinsic = Eigen::Matrix4d::Identity();
    translation_only_extrinsic.block<3, 1>(0, 3) = extrinsic.block<3, 1>(0, 3);

    return is_success ? translation_only_extrinsic : Eigen::Matrix4d::Identity();
}

Eigen::Matrix4d TransformationEstimationPointToPlane::ComputeRotation(
    const geometry::PointCloud &source,
    const geometry::PointCloud &target,
    const CorrespondenceSet &corres) const {
    if (corres.empty() || !target.HasNormals())
    ...
    double & r, double & w) {
        ...
        J_r.block<3, 1>(3, 0) = Eigen::Vector3d::Zero();
    };
    ...
    // Extract rotation and form a matrix with obtained rotation and zero translation
    Eigen::Matrix4d rotation_only_extrinsic = Eigen::Matrix4d::Identity();
    rotation_only_extrinsic.block<3, 3>(0, 0) = extrinsic.block<3, 3>(0, 0);

    return is_success ? rotation_only_extrinsic : Eigen::Matrix4d::Identity();
}
